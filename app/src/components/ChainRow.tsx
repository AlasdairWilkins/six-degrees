...
export default ({
  ...
  const invalidClasses = useMemo(() => {
    ...
    const relevantLinks = invalidLinks.filter(
      ({ movieId, personId }) =>
        movie?.id === movieId || person?.id === personId,
    );

    const fromActorInvalid = relevantLinks.some(
      ({ movieId, personId }) =>
        movieId === movie?.id && person?.id !== personId,
    );
    const movieInvalid = relevantLinks.some(
      ({ movieId }) => movieId === movie?.id,
    );
    const toActorInvalid = relevantLinks.some(
  ...
  const updateChain = useCallback(
    (item: { movie: Movie | null } | { person: Person | null }) => {
      if ("movie" in item) {
  ...
  if (isSubmitted) {
    return (
      <div className="chain-row">
        <span className={invalidClasses[0]}>
          {fromActor ? fromActor.name : "Who"} was in{" "}
        </span>
    ...
    <div className="chain-row">
      ...
      <span>
        <MovieSearchAutocomplete
          ...
          usedIds={usedMovieIds}
          onSelect={(movie) => updateChain({ movie })}
        />
      </span>
      <span>with</span>
      <PersonSearchAutocomplete
        value={person}
        disabled={isSubmitted}
        usedIds={usedPersonIds}
        onSelect={(person) => updateChain({ person })}
      />
      {!isSubmitted && (
        <>
          {addRow && (
            <button disabled={person?.id === targetActorId} onClick={addRow}>
              +
            </button>
          )}
