...
export default ({
  ...
  const {
    ...
    removeUsedIds,
    updateUsedIds,
    usedIds: { movie: usedMovieIds, person: usedPersonIds },
  } = useHandleUsedIds({ initialActor, completedChains, targetActorId });
  ...
  const onSubmit = useCallback(() => {
    ...
    const submitHandler = async (payload: Payload) => {
      const response = await fetchHandler("/api/validate-chain", {
        method: "POST",
    ...
    const links: Payload["links"] = [initialActor, ...chains.flat()].map(
      (entry, index) => {
  ...
  const onPlayAgain = useCallback(() => {
    assertNonNull(
      initialActor,
      "Cannot complete a chain without an initial actor",
    );
  ...
  const insertChain = useCallback(
    (index: number, chain: Chain = [null, null]) => {
      insertUsedIds(chain);
  ...
  const canSubmit = useMemo(() => {
    return (
      chains.flat().every((value) => !!value) &&
      chains[chains.length - 1][1]?.id === targetActorId
    );
  }, [chains]);
      ...
      {chains.map((chain, index) => {
        ...
        return (
          <ChainRow
            ...
            {...(fromActor ? { fromActor } : {})}
            {...(chains.length < 6
              ? { addRow: () => insertChain(index + 1) }
              : {})}
            {...(chains.length > 1
              ? { removeRow: () => removeChain(index) }
              : {})}
          />
        );
      })}
